        ...
        _this.handleSwitchPages = function (direction) {
            return function () {
                ...
                if (typeof set === 'function') {
                    set(function (list) {
                        _this.setState({
                            page: direction === "prev" ? Math.abs(list.length + page - 1) % list.length : (page + 1) % list.length
                        });
                    });
                } else {
                    list = set;
...
var _initialiseProps = function _initialiseProps() {
    ...
    this.buildSet = function (props) {
        ...
        if (set && set.constructor === Array && set.length > 1) {
            _this3.state.set = set;
        } else if (typeof set === 'function') {
            set(function (list) {
                _this3.state.set = list;
            });
        } else {
            _this3.state.set = [{ src: src, alt: alt, txt: txt }];
        }
    };
